...
public class AddUserStage {
    ...
    private static void initButtons() {
        ...
        addButton = new Button("Dodaj") {{
            setOnAction(e -> {
                int result = RSA.createRsaForUser(emailTextField.getText(), passTextField.getText());
                if (result == 0)
    ...
    private static void pinComponents() {
        VBox layout = new VBox(10) {{
            getChildren().addAll(label1, emailTextField, label2, passTextField, addButton, closeButton);
